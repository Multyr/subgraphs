...
test("handleWithdraw updates position and creates transaction", () => {
  ...
  handleDeposit(createDepositEvent(vault, user, user, BigInt.fromI32(500), BigInt.fromI32(50)))
  handleWithdraw(createWithdrawEvent(vault, user, user, user, BigInt.fromI32(200), BigInt.fromI32(20)))
...
test("handleTransfer moves shares between positions and creates transaction", () => {
  ...
  handleTransfer(createTransferEvent(vault, from, to, BigInt.fromI32(10)))
...
test("VaultDayData.apy updates across days and UserPositionDayData.sharePrice mirrors vault", () => {
  ...
  dep1.block.timestamp = BigInt.fromI32(86400) // day 1
  handleDeposit(dep1)
...
test("Transfer with mint/burn (from/to zero) is ignored", () => {
  ...
  handleTransfer(createTransferEvent(vault, Address.zero(), user, BigInt.fromI32(5)))
  handleTransfer(createTransferEvent(vault, user, Address.zero(), BigInt.fromI32(5)))
